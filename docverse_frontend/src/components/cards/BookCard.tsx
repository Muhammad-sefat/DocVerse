        ...
        >
          ...
          <div className="flex flex-1 flex-col justify-between">
            <div>
              ...
              <p className="mt-0.5 text-sm text-secondary-500">{book.author.name}</p>
              <Rating value={book.rating} showValue className="mt-1" />
            </div>
            <div className="flex items-center justify-between">
              <span className="font-bold text-primary-700">{formatPrice(book.price)}</span>
      ...
      <Link href={`/books/${book.id}`}>
        ...
        >
          ...
          <div className="flex flex-col gap-1.5 p-3">
            <Badge variant="secondary" className="w-fit">{book.category.name}</Badge>
            <h3 className="font-semibold text-secondary-900 line-clamp-1 group-hover:text-primary-700 transition-colors">
              {book.title}
            </h3>
      ...
      >
        ...
        <div className="flex flex-col gap-2 p-4">
          ...
          <div className="mt-1 flex items-center justify-between">
            <div>
              <span className="text-lg font-bold text-primary-700">{formatPrice(book.price)}</span>
              <span className="ml-2 text-sm text-secondary-400 line-through">
                {formatPrice(book.price * 1.2)}
              </span>
            </div>
            <span className="text-xs text-secondary-400">{book.totalPages} pages</span>
          </div>
